...
- 导演模式在角色分析前提供结构清洗、可选 LLM 配音改写和逐段双栏校对，原文始终保留；
...
1. **导入**：粘贴文章、小说或剧本，选择原文、目标语言和预处理模式；
2. **文本预处理**：生成逐段校对稿，原始导入文本保持不变；
3. **确认与分析**：用户确认预处理稿后，LLM 才分析说话人、旁白、舞台说明和语句边界；
4. **角色复核**：拖动角色到单句或多选语句，也可用下拉框；低置信度语句必须显式确认；
5. **翻译校对**：逐句检查目标语言台词和供 IndexTTS2 使用的中文情绪参考文本；
6. **音色映射与生成**：为每个实际配音角色选择角色预设，确认后才冻结快照并开始推理。

预处理提供三种模式：

- **结构清洗（默认）**：完全本地运行，统一段落结构，并在整篇范围识别成对引号和引号间旁白；
- **配音改写**：先做本地结构清洗，再按稳定单元让 LLM 做不增删剧情信息的等义改写；单段
  请求或校验失败只回退该段，不阻断其他段落；
- **跳过预处理**：原样进入校对，仅用于兼容和调试，复杂排版可能产生不理想分块。

校对区保留只读原文和可编辑当前稿，支持保存、恢复原文、恢复本地清洗稿及单段重跑 LLM。
刷新页面后当前项目、滚动位置和未保存草稿仍保留；有未保存段落时不能确认。后续分析、翻译
和配音只使用用户确认后的文本。纯空白、纯标点、纯引号和停顿标记会保留在时间线中，但不
会进入翻译或 TTS；类似“对白”+动作描写+“对白”的句子会在本地按引号边界拆成独立单元。

系统默认识别旁白，项目顶部开关可以整体关闭旁白配音，但原文和时间线行不会被删除。
角色预设的基础 WAV 会复制进本地托管库，可使用长音频；它不受 GSV 参考音频的 3–10 秒
窗口限制。预处理、分析与翻译采用单项目单飞后台命令；重复点击不会重复请求 LLM，服务重启
后会把中断阶段标为可重试错误。导演模式的 LLM 实时活动窗口会同时显示预处理、分析和翻译。
