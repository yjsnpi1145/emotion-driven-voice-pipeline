...
遥测或浏览器端模型调用；它只访问 loopback `/api/v1`。界面沿用 GPT-SoVITS 整合包的
页签、折叠面板、紧凑表单和蓝色主操作风格，并分为四个长期保留的工作区：

- **配音工作台**：创建整篇任务、查看章节/分块状态、试听 reference/GSV、编辑草稿、执行
  三种局部重生成和显式重拼接；
- **模型管理**：通过 Windows 原生文件选择器选择 `.ckpt`/`.pth`，导入训练模型、显式切换
  当前模型，并可从界面打开模型库、模型来源目录或单个模型档案文件夹；
- **LLM 设置**：保存并热切换 OpenAI 兼容的 Base URL、模型、超时、重试及参考文本修正次数，
  可先测试候选配置。API key 单独保存在 `runtime/state`，GET 响应与页面永不回显明文；
- **系统状态**：集中查看控制面、IndexTTS2、GPT-SoVITS、SQLite、调度器和 GPU 串行队列，
  并可打开本地音频产物与日志目录。

可编辑中文参考文本、合成文本、速度、停顿、种子和八维情绪向量。保存仅更新草稿，绝不
自动排队推理；“恢复 LLM 值”只将当前向量复原为该段已保存的 LLM 向量。任务进度通过章节
SSE 更新，连接中断时页面回退为本地 REST 刷新。文件夹打开与文件选择 API 只接受枚举资源
和扩展名，不接受浏览器提交的任意路径。
